...
the binary vulnerable-vs-secure probe restricted to each language stratum (C, PHP, JS)
...
def probe_subset(safe_mat: np.ndarray, vuln_mat: np.ndarray,
                 n_components: int = 50, cv: int = 5) -> dict:
...
def make_figure(results: dict, global_mean: dict, out_path: Path):
    """
    One panel per language (C, PHP, JS), showing:
    ...
    fig, axes = plt.subplots(1, len(langs), figsize=(3.5 * len(langs), 3.2), sharey=True)
    if len(langs) == 1:
        axes = [axes]
    ...
    for ax, lang in zip(axes, langs):
        ...
        ax.plot(xs, aucs, "s-", color=LANG_COLORS[lang], lw=1.5, ms=5,
                label=f"{lang} (n={n})")
        ...
        ax.plot(g_xs, g_aucs, "o--", color="#888888", lw=1.0, ms=3,
                label="Global mean-token", alpha=0.6)
...
def print_table(results: dict, global_mean: dict):
    ...
    print("=" * 100)
    header = f"{'Layer':>6}  " + "  ".join(f"{l:^27}" for l in langs) + f"  {'Global':^27}"
    print(header)
    print("-" * 100)
    for layer in LAYERS:
        row = f"  L{layer:>2}   "
...
def main():
    ...
    for layer in LAYERS:
        ...
        print(f"  Global:  AUROC {res_all['roc_auc']:.3f} [{res_all['ci_lo']:.3f}–{res_all['ci_hi']:.3f}]  (n={res_all['n']})")
        ...
        for lang, mask in lang_masks.items():
            ...
            print(f"  {lang:4s}: AUROC {res['roc_auc']:.3f} [{res['ci_lo']:.3f}–{res['ci_hi']:.3f}]  (n={res['n']})")
